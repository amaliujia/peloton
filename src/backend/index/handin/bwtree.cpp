...
//    KeyEqualityChecker>
//    BWTree<KeyType, ValueType, KeyComparator,
//    KeyEqualityChecker>::KeyComparator comparator_;
//    template<typename KeyType, typename ValueType, class KeyComparator, class
//    KeyEqualityChecker>
//    BWTree<KeyType, ValueType, KeyComparator,
//    KeyEqualityChecker>::KeyEqualityChecker key_equality_checker_;

template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
void BWTree<KeyType, ValueType, KeyComparator,
            KeyEqualityChecker>::SplitInnerNodeUtil(const BWNode *node_ptr,
                                                    KeyType &split_key,
                                                    PID &right_pid,
  ...
  while (cur_ptr->GetType() != NInner) {
    if (cur_ptr->GetType() == NSplitEntry) {
      const BWSplitEntryNode<KeyType> *split_entry_ptr =
          static_cast<const BWSplitEntryNode<KeyType> *>(cur_ptr);
  ...
  // come to inner node
  const BWInnerNode<KeyType> *inner_ptr =
      static_cast<const BWInnerNode<KeyType> *>(cur_ptr);
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
void BWTree<KeyType, ValueType, KeyComparator,
            KeyEqualityChecker>::SplitLeafNodeUtil(const BWNode *, KeyType &,
                                                   PID &,
                                                   std::vector<KeyType> &,
                                                   std::vector<ValueType> &) {
  // TODO this function cannot pass compiler, fix it later.
  /* SplitLeafNodeUtil(const BWNode *node_ptr, KeyType &split_key, PID
     &right_pid,
     std::vector<KeyType> &keys,
  ...
  while(cur_ptr->GetType()!=NLeaf) {
    if(cur_ptr->GetType()==NInsert) {
      const BWInsertNode<KeyType, ValueType> *insert_ptr = static_cast<const
  BWInsertNode<KeyType, ValueType> *>(cur_ptr);
      ...
      const BWDeleteNode<KeyType> *delete_ptr = static_cast<const
  BWDeleteNode<KeyType> *>(cur_ptr);
  ...
  const BWLeafNode<KeyType, ValueType, KeyComparator> *leaf_ptr =
  static_cast<const BWLeafNode<KeyType, ValueType, KeyComparator> *>(cur_ptr);
...
bool BWTree<KeyType, ValueType, KeyComparator,
            KeyEqualityChecker>::SplitInnerNode(PID cur, const BWNode *node_ptr,
                                                KeyType &split_key,
                                                PID &right_pid) {
  std::vector<KeyType> keys;
  std::vector<PID> pids;
  ...
  split_ptr = new BWSplitNode<KeyType>(node_ptr->GetSlotUsage() - keys.size(),
                                       node_ptr->GetChainLength() + 1, node_ptr,
                                       split_key, right_pid, old_right);
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
bool BWTree<KeyType, ValueType, KeyComparator,
            KeyEqualityChecker>::SplitLeafNode(PID cur, const BWNode *node_ptr,
                                               KeyType &split_key,
                                               PID &right_pid) {
  std::vector<KeyType> keys;
  std::vector<ValueType> values;
  ...
  BWNode *right_ptr = NULL;
  right_ptr = new BWLeafNode<KeyType, ValueType, KeyComparator>(keys, values,
                                                                cur, old_right);
  right_pid = pidTable.allocate_PID(right_ptr);
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
bool BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker>::Split(
    PID cur, const BWNode *node_ptr, KeyType &split_key, PID &right_pid,
    NodeType right_type) {
  if (right_type == NLeaf) {
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
bool BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker>::DupExist(
    const BWNode *node_ptr, const KeyType &key) {
  ...
  do {
    ...
    if (node_type == NLeaf) {
      const BWLeafNode<KeyType, ValueType, KeyComparator> *leaf_node_ptr =
          static_cast<const BWLeafNode<KeyType, ValueType, KeyComparator> *>(
              node_ptr);
      ...
      const BWInsertNode<KeyType, ValueType> *insert_node_ptr =
          static_cast<const BWInsertNode<KeyType, ValueType> *>(node_ptr);
      if (key_equality_checker_(insert_node_ptr->GetKey(), key) &&
          (++delta > 0)) {
        return true;
      }
    } else if (node_type == NDelete) {
      const BWDeleteNode<KeyType> *delete_node_ptr =
          static_cast<const BWDeleteNode<KeyType> *>(node_ptr);
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
bool BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker>::DeltaInsert(
    PID cur, BWNode *node_ptr, const KeyType &key, const ValueType &value) {
  PIDTable &pidTable = PIDTable::get_table();
  BWInsertNode<KeyType, ValueType> *insert_node_ptr =
      new BWInsertNode<KeyType, ValueType>(node_ptr->GetSlotUsage() + 1,
                                           node_ptr->GetChainLength() + 1,
                                           node_ptr, key, value);
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
bool BWTree<KeyType, ValueType, KeyComparator,
            KeyEqualityChecker>::InsertEntryUtil(KeyType key, ValueType value,
                                                 std::vector<PID> &path,
                                                 PID cur) {
  ...
  while (true) {
    ...
    if (node_ptr->GetType() == NSplit) {
      BWSplitNode<KeyType> *split_ptr =
          static_cast<BWSplitNode<KeyType> *>(node_ptr);
      InsertSplitEntry(path, split_ptr->GetSplitKey(),
                       split_ptr->GetRightPID());
      // continue;
    }
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker>::ScanKeyUtil(
    PID cur, KeyType key, std::vector<ValueType> &ret) {
  PIDTable &pidTable = PIDTable::get_table();
  ...
  if (node_ptr->GetType() == NLeaf) {
    ...
    while (true) {
      ...
      if (node_ptr->GetType() == NSplit) {
        const BWSplitNode<KeyType> *split_ptr =
            static_cast<const BWSplitNode<KeyType> *>(node_ptr);
        KeyType split_key = split_ptr->GetSplitKey();
        if (comparator_(key, split_key) ==
            true) {  // == true means key < split_key
          node_ptr = split_ptr->GetNext();
        } else {
          node_ptr = pidTable.get(split_ptr->GetRightPID());
        }
      } else if (node_ptr->GetType() == NInsert) {
        const BWInsertNode<KeyType, ValueType> *insert_ptr =
            static_cast<const BWInsertNode<KeyType, ValueType> *>(node_ptr);
        if (key_equality_checker_(insert_ptr->GetKey(), key) == true &&
            delete_count == 0) {
          ret.push_back(insert_ptr->GetValue());
          //                if(!Duplicate) {
          //                  break;
          //                }
        } else if (key_equality_checker_(insert_ptr->GetKey(), key) == true &&
                   delete_count > 0) {
          delete_count--;
        }
        ...
        const BWDeleteNode<KeyType> *delete_ptr =
            static_cast<const BWDeleteNode<KeyType> *>(node_ptr);
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
const BWInnerNode<KeyType> *
BWTree<KeyType, ValueType, KeyComparator,
       KeyEqualityChecker>::ConstructConsolidatedInnerNode(const BWNode *
                                                               node_chain) {
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
const BWLeafNode<KeyType, ValueType, KeyComparator> *
BWTree<KeyType, ValueType, KeyComparator,
       KeyEqualityChecker>::ConstructConsolidatedLeafNode(const BWNode *
                                                              node_chain) {
  std::vector<KeyType> keys;
  std::vector<ValueType> values;
  PID left, right;
  ConstructConsolidatedLeafNodeInternal(node_chain, keys, values, left, right);
  return new BWLeafNode<KeyType, ValueType, KeyComparator>(keys, values, left,
                                                           right);
}
...
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker>::
    ConstructConsolidatedLeafNodeInternal(const BWNode *node_chain,
                                          std::vector<KeyType> &keys,
                                          std::vector<ValueType> &values,
                                          PID &left, PID &right) {
  if (node_chain->GetType() == NLeaf) {
    // arrive at bottom
    const BWLeafNode<KeyType, ValueType, KeyComparator> *node =
        static_cast<const BWLeafNode<KeyType, ValueType, KeyComparator> *>(
            node_chain);
    keys = node->GetKeys();
  ...
  switch (node_chain->GetType()) {
    case NInsert:
      ConsolidateInsertNode(
          static_cast<const BWInsertNode<KeyType, ValueType> *>(node_chain),
          keys, values);
      break;
    case NDelete:
      // not implemented
    ...
    case NSplit:
      ConsolidateSplitNode(
          static_cast<const BWSplitNode<KeyType> *>(node_chain), keys, values);
      break;
    case NMerge:
      assert(0);
      break;
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker>::
    ConstructConsolidatedInnerNodeInternal(const BWNode *node_chain,
                                           std::vector<KeyType> &keys,
                                           std::vector<PID> &children,
                                           PID &left, PID &right) {
  if (node_chain->GetType() == NInner) {
    // arrive at bottom
    const BWInnerNode<KeyType> *node =
        static_cast<const BWInnerNode<KeyType> *>(node_chain);
    keys = node->GetKeys();
  ...
  switch (node_chain->GetType()) {
    case NSplit:
      ConsolidateSplitNode(
          static_cast<const BWSplitNode<KeyType> *>(node_chain), keys,
          children);
      break;
    case NSplitEntry:
      ConsolidateSplitEntryNode(
          static_cast<const BWSplitEntryNode<KeyType> *>(node_chain), keys,
          children);
      break;
    case NMerge:
      // not implemented
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker>::
    ConsolidateInsertNode(const BWInsertNode<KeyType, ValueType> *node,
                          std::vector<KeyType> &keys,
                          std::vector<ValueType> &values) {
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker>::
    ConsolidateSplitNode(const BWSplitNode<KeyType> *node,
                         std::vector<KeyType> &keys,
                         std::vector<ValueType> &values) {
  assert(keys.size() == values.size());
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker>
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker>::
    ConsolidateSplitEntryNode(const BWSplitEntryNode<KeyType> *node,
                              std::vector<KeyType> &keys,
                              std::vector<PID> &children) {
  ...
  // TODO key belongs to left or right?
  auto position =
      std::lower_bound(keys.begin(), keys.end(), high_key, comparator_);
  assert(position != keys.begin() &&
         key_equality_checker_(*position, high_key));
  auto dist = std::distance(keys.begin(), position);
