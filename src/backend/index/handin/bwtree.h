...
class BWNode {
 public:
  ...
  inline bool IfInnerNode() const { return !if_leaf_; }
 ...
 protected:
  ...
  BWNode(const size_type &slot_usage, const size_type &chain_length,
         bool if_leaf)
...
class BWNormalNode : public BWNode {
 public:
  ...
  // TODO ???
  inline const BWNode *GetNext() const { return NULL; };

  inline PID GetLeft() const { return left_; }

  inline PID GetRight() const { return right_; }

 protected:
  BWNormalNode(const size_type &slot_usage, const size_type &chain_length,
               bool if_leaf, const PID &left, const PID &right)
      : BWNode(slot_usage, chain_length, if_leaf), left_(left), right_(right) {}
...
class BWInnerNode : public BWNormalNode<KeyType> {
 public:
  ...
  /*BWInnerNode(std::vector<KeyType> &keys, std::vector<PID> &children, const
  PID &left, const PID &right):
          BWNormalNode<KeyType>(keys.size(), 0, false, left, right),
  keys_(std::move(keys)), children_(std::move(children)) {
  ...
  inline const std::vector<PID> &GetPIDs() const { return GetChildren(); }
...
class BWLeafNode : public BWNormalNode<KeyType> {
 public:
  ...
  /*BWLeafNode(std::vector<KeyType> &keys, std::vector<ValueType> &values, const
     PID &left, const PID &right):
          BWNormalNode<KeyType>(keys.size(), 0, true, left, right),
  ...
  void ScanKey(const KeyType &, const KeyComparator &,
...
class BWDeltaNode : public BWNode {
 public:
  ...
  inline const BWNode *GetNext() const { return next_; }
 ...
 protected:
  BWDeltaNode(const size_type &slot_usage, const size_type &chain_length,
              const BWNode *next)
      : BWNode(slot_usage, chain_length, false), next_(next) {}
  const BWNode *next_;
};
...
class BWInsertNode : public BWDeltaNode {
 public:
  BWInsertNode(const size_type &slot_usage, const size_type &chain_length,
               const BWNode *next, const KeyType &key, const ValueType &value)
...
class BWDeleteNode : public BWDeltaNode {
 public:
  ...
  BWDeleteNode(const size_type &slot_usage, const size_type &chain_length,
               const BWNode *next, const KeyType &key)
...
class BWSplitNode : public BWDeltaNode {
 public:
  ...
  inline const KeyType &GetSplitKey() const { return key_; }

  inline PID GetRightPID() const { return right_; }

  BWSplitNode(const size_type &slot_usage, const size_type &chain_length,
              const BWNode *next, const KeyType &key, PID right, PID old)
...
class BWSplitEntryNode : public BWDeltaNode {
 public:
  BWSplitEntryNode(const size_type &slot_usage, const size_type &chain_length,
                   const BWNode *next, const KeyType &low, const KeyType &high,
                   PID to)
      : BWDeltaNode(slot_usage, chain_length, next),
        low_key_(low),
        high_key_(high),
  ...
  inline const KeyType &GetHightKey() const { return high_key_; }

  inline PID GetNextPID() const { return to_; }
...
class BWRemoveNode : public BWDeltaNode {
 public:
  ...
  inline NodeType GetType() const { return NRemove; }
};
...
class BWMergeNode : public BWDeltaNode {
 public:
  ...
  inline NodeType GetType() const { return NMerge; }
...
class MergeEntryNode : public BWDeltaNode {
 public:
  ...
  inline NodeType GetType() const { return NMergeEntry; }
};

enum OpType { OInsert, ODelete, OUnknown };
...
class InsertOperation : public Operation {
 public:
  OpType GetOpType() { return OInsert; }
};

class DeleteOperation : public Operation {
 public:
  OpType GetOpType() { return ODelete; }
};
...
class PIDTable {
 ...
 private:
  ...
  inline bool is_first_PID(PID pid) { return (pid & second_level_mask) == 0; }
...
class BWTree {
 ...
 private:
  int BinaryGreaterThan(std::vector<KeyType> &arr, KeyType target,
                        KeyComparator comp) {
 ...
 private:
  void InsertSplitEntry(const std::vector<PID> &path, const KeyType &low_key,
                        const PID &right_pid) {
      ...
      while (true) {
        ...
        while (cur_ptr->GetType() != NInner) {
          if (cur_ptr->GetType() == NSplitEntry) {
            const BWSplitEntryNode<KeyType> *split_ptr =
        ...
        BWNode *split_entry_ptr = new BWSplitEntryNode<KeyType>(
            slot_usage + 1, chain_len + 1, parent_ptr, low_key, low_key,
            right_pid);

        PID split_entry_pid = pid_table.allocate_PID(split_entry_ptr);

        bool ret = pid_table.bool_compare_and_swap(path[path.size() - 2],
                                                   parent_ptr, split_entry_ptr);
        if (ret == false) {
  ...
  const BWInnerNode<KeyType> *ConstructConsolidatedInnerNode(
      const BWNode *node_chain);

  void ConstructConsolidatedLeafNodeInternal(const BWNode *node_chain,
                                             std::vector<KeyType> &keys,
                                             std::vector<ValueType> &values,
                                             PID &left, PID &right);

  const BWLeafNode<KeyType, ValueType, KeyComparator> *
  ConstructConsolidatedLeafNode(const BWNode *node_chain);

  void ConstructConsolidatedInnerNodeInternal(const BWNode *node_chain,
                                              std::vector<KeyType> &keys,
                                              std::vector<PID> &children,
                                              PID &left, PID &right);

  void ConsolidateInsertNode(const BWInsertNode<KeyType, ValueType> *node,
                             std::vector<KeyType> &keys,
                             std::vector<ValueType> &values);

  void ConsolidateSplitNode(const BWSplitNode<KeyType> *node,
                            std::vector<KeyType> &keys,
                            std::vector<ValueType> &values);
  ...
  void ConsolidateSplitEntryNode(const BWSplitEntryNode<KeyType> *node,
                                 std::vector<KeyType> &keys,
                                 std::vector<PID> &children);
  ...
  /*
   *  In inner levels, the delta nodes should be SplitEntryNode, MergeEntryNode,
   *SplitNode.
   ...
   */
  void SplitInnerNodeUtil(const BWNode *node_ptr, KeyType &split_key,
                          PID &right_pid, std::vector<KeyType> &keys,
                          std::vector<PID> &pids);

  // TODO: handle duplicate keys
  void SplitLeafNodeUtil(const BWNode *node_ptr, KeyType &split_key,
                         PID &right_pid, std::vector<KeyType> &keys,
                         std::vector<ValueType> &values);

  // Duplicate keys handled
  bool SplitInnerNode(PID cur, const BWNode *node_ptr, KeyType &split_key,
                      PID &right_pid);

  bool SplitLeafNode(PID cur, const BWNode *node_ptr, KeyType &split_key,
                     PID &right_pid);

  bool Split(PID cur, const BWNode *node_ptr, KeyType &split_key,
             PID &right_pid, NodeType right_type);

  bool DupExist(const BWNode *node_ptr, const KeyType &key);

  bool DeltaInsert(PID cur, BWNode *node_ptr, const KeyType &key,
  ...
  PID GetNextPID(PID cur, KeyType key) {
    ...
    while (cur_ptr->GetType() != NInner) {
      if (cur_ptr->GetType() == NSplitEntry) {
        ...
        if (!comparator_(key, split_entry_ptr->GetLowKey()) &&
            comparator_(key, split_entry_ptr->GetHightKey())) {
    ...
    // const BWInnerNode<KeyType> *inner_ptr = static_cast<const
    // BWInnerNode<KeyType> *>(cur_ptr);
 ...
 private:
  bool InsertEntryUtil(KeyType key, ValueType value, std::vector<PID> &path,
                       PID cur);
 ...
 public:
  BWTree(IndexMetadata *indexMetadata)
      : comparator_(indexMetadata), key_equality_checker_(indexMetadata) {}
  ...
  void ScanAllKeys(std::vector<ValueType> &) {
    ...
    while (node_ptr->GetType() != NLeaf) {
      const BWInnerNode<KeyType> *cur_ptr =
          static_cast<const BWInnerNode<KeyType> *>(node_ptr);
