...
void main() {
  ...
  for (int iter = 0; iter < 3; ++iter) {
    ...
    for (int i = 0; i < size; ++i) {
      ...
      new_garbage =
          index::BWBaseNode::GenerateRandomNodeChain((rand() % max_length) + 1);
  ...
  for (int i = 0; i < size; ++i) {
    if (i % 100 == 0) {
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(rand() % max_duration));
      registered_time = gc.Register();
    }
    new_garbage =
        index::BWBaseNode::GenerateRandomNodeChain((rand() % max_length) + 1);
...
void VerifyAndFree(
    ...
    const std::vector<index::PIDTable<
        index::IntsKey<1>, index::IntsComparator<1>>::Address> &addresses,
  ...
  // free
  for (size_t i = 0; i < addresses.size(); ++i) table.free_PID(pids[i]);
}
...
void main() {
  ...
  // PIDTable
  index::PIDTable<index::IntsKey<1>, index::IntsComparator<1>> table;
  std::vector<index::PIDTable<index::IntsKey<1>,
                              index::IntsComparator<1>>::Address> addresses;
  ...
  for (int iter = 0; iter < 4; ++iter) {
    // allocate
    for (int i = 0; i < N; ++i) {
      addresses.push_back((index::PIDTable<index::IntsKey<1>,
                                           index::IntsComparator<1>>::Address)(
          unsigned long)rand());
...
void Allocate(
    std::atomic<std::uint_least8_t> *no_gen, int size_each,
    const std::vector<index::PIDTable<
        index::IntsKey<1>, index::IntsComparator<1>>::Address> *addresses,
    std::vector<index::PID> *pids,
    index::PIDTable<index::IntsKey<1>, index::IntsComparator<1>> *table) {
  ...
  for (int i = 0; i < size_each; ++i) {
    (*pids)[size_each * no + i] =
        table->allocate_PID((*addresses)[size_each * no + i]);
    if (i % 100 == 0)
  ...
  for (int iter = 0; iter < 5; ++iter) {
    for (int i = 0; i < size; ++i) {
      addresses.push_back((index::PIDTable<index::IntsKey<1>,
                                           index::IntsComparator<1>>::Address)(
          unsigned long)rand());
      pids.push_back(index::PIDTable<index::IntsKey<1>,
                                     index::IntsComparator<1>>::PID_NULL);
    }

    LaunchParallelTest(total, Allocate, &no_gen, size_each, &addresses, &pids,
                       &table);
    ...
    for (int i = 0; i < total; ++i) {
      if (i % 2 == 1) {
        for (int j = 0; j < size_each; ++j)
          pids[i * size_each + j] =
              table.allocate_PID(addresses[i * size_each + j]);
      }
    }

    LaunchParallelTest(total, AllocateOrFreeAndTest, &no_gen, size_each,
                       &addresses, &pids, &table);
...
void CompareAndSwap(
    std::atomic<std::uint_least8_t> *no_gen, int size_each,
    const std::vector<
        index::PIDTable<index::IntsKey<1>, index::IntsComparator<1>>::Address> *
        original_addresses,
    const std::vector<index::PIDTable<
        index::IntsKey<1>, index::IntsComparator<1>>::Address> *to_addresses,
    std::vector<bool> *success, std::vector<index::PID> *pids,
    index::PIDTable<index::IntsKey<1>, index::IntsComparator<1>> *table) {
  auto no = (*no_gen)++;
  for (int i = 0; i < size_each; ++i) {
    (*success)[size_each * no + i] = table->bool_compare_and_swap(
...
void main4() {
  ...
  // PIDTable
  index::PIDTable<index::IntsKey<1>, index::IntsComparator<1>> table;
  std::vector<index::PIDTable<
      index::IntsKey<1>, index::IntsComparator<1>>::Address> original_addresses;
  std::vector<index::PIDTable<index::IntsKey<1>,
                              index::IntsComparator<1>>::Address> to_addresses;
  std::vector<index::PID> pids;
  ...
  for (int iter = 0; iter < 5; ++iter) {
    // initialize
    for (int i = 0; i < size; ++i) {
      original_addresses.push_back(
          (index::PIDTable<index::IntsKey<1>,
                           index::IntsComparator<1>>::Address)(
              unsigned long)rand());
    ...
    // launch test
    LaunchParallelTest(total, CompareAndSwap, &no_gen, size_each,
                       &original_addresses, &to_addresses, &success, &pids,
                       &table);
    ...
    // clean up
    for (int i = 0; i < size; ++i) table.free_PID(pids[i]);
    no_gen = 0;
...
void ConcurrentCompareAndSwap(
    std::atomic<std::uint_least8_t> *no_gen, int size,
    const std::vector<
        index::PIDTable<index::IntsKey<1>, index::IntsComparator<1>>::Address> *
        original_addresses,
    const std::vector<index::PIDTable<
...
void main5() {
  ...
  for (int iter = 0; iter < 5; ++iter) {
    ...
    // launch test
    LaunchParallelTest(total, ConcurrentCompareAndSwap, &no_gen, size,
                       &original_addresses, to_addresses, success, &pids,
                       &table);
    ...
    for (int i = 0; i < size; ++i) {
      ...
      for (int j = 0; j < total; ++j) {
        EXPECT_TRUE(
            (success[j][i] && table.get(pids[i]) == to_addresses[j][i]) ||
            !success[j][i]);
        if (success[j][i]) only_one = !only_one;
      }
...
void GenerateKeyValues(
    ...
    int size, int key_value_repeat_factor, int key_repeat_factor,
    bool shuffle) {
...
void InsertFunction(
    std::atomic<std::uint_least8_t> *no_gen, index::Index *index,
    const size_t size_each,
    std::vector<std::pair<std::unique_ptr<storage::Tuple>, ItemPointer>> *pairs,
...
void main2() {
  ...
  for (int iter = 0; iter < 1; ++iter) {
    ...
    // try insertion
    LaunchParallelTest(1, InsertFunction, &no_gen, index.get(), size, &pairs,
                       &result);
    ...
    index->Cleanup();
    LOG_DEBUG("MemoryFootprint after insertion: %lu",
              (unsigned long)index->GetMemoryFootprint());

    // try deletion
    no_gen = 0;
    LaunchParallelTest(1, DeleteFunction, &no_gen, index.get(), size, &pairs,
                       &result);
    ...
    index->Cleanup();
    LOG_DEBUG("MemoryFootprint after deletion: %lu",
              (unsigned long)index->GetMemoryFootprint());
...
void main3() {
  ...
  if (!index->HasUniqueKeys()) {
    LOG_TRACE(
        "Index is built in duplicate mode, unique key test will not be done.");
    return;
  }
  ...
  for (int iter = 0; iter < 1; ++iter) {
    ...
    index->Cleanup();
    LOG_DEBUG("MemoryFootprint after first insertion: %lu",
              (unsigned long)index->GetMemoryFootprint());
    ...
    index->Cleanup();
    LOG_DEBUG("MemoryFootprint after second insertion: %lu",
              (unsigned long)index->GetMemoryFootprint());
...
void main4() {
  ...
  for (int iter = 0; iter < 1; ++iter) {
    ...
    // try insertion
    LaunchParallelTest(thread_number, InsertFunction, &no_gen, index.get(),
                       size_each, &pairs, &result);
    ...
    // try deletion
    no_gen = 0;
    LaunchParallelTest(thread_number, DeleteFunction, &no_gen, index.get(),
                       size_each, &pairs, &result);
