...
void *Begin(void *arg) {
  ...
  while (!(gc->stopped_)) {
    std::this_thread::sleep_for(
        std::chrono::milliseconds(GarbageCollector::epoch_interval_));
...
const BWBaseNode *BWBaseNode::GenerateRandomNodeChain(int length) {
  return BWNode<index::IntsKey<1>,
                index::IntsComparator<1>>::GenerateRandomNodeChain(length);
}

template <typename KeyType, typename KeyComparator>
const BWNode<KeyType, KeyComparator> *
BWNode<KeyType, KeyComparator>::GenerateRandomNodeChain(int length) {
...
template <typename KeyType, typename KeyComparator>
const BWNode<KeyType, KeyComparator> *
BWNode<KeyType, KeyComparator>::GenerateRandomNode(
    NodeType type, const BWNode<KeyType, KeyComparator> *next) {
  switch (type) {
    case NInner:
    ...
    case NInsert:
      ...
      return new BWInsertNode<IntsKey<1>, IntsComparator<1>, ItemPointer>(
          next, 1, PIDTable<KeyType, KeyComparator>::PID_NULL,
          PIDTable<KeyType, KeyComparator>::PID_NULL, IntsKey<1>(),
          ItemPointer());
    case NDelete:
      myassert(next != nullptr);
...
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
            ...
            Duplicate>::SubmitGarbageNode(const BWNode<KeyType, KeyComparator> *
                                              node) {
  ...
  if (node->IfInnerNode()) {
    ...
    CreateInnerNodeView(node, &keys_view, &children_view, &left_view,
                        &right_view);
    for (auto child = children_view.begin(); child != children_view.end();
         ++child) {
      const BWNode<KeyType, KeyComparator> *child_node =
          pid_table_.get(*(child));
...
size_t BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
              ValueComparator, ValueEqualityChecker,
              Duplicate>::GetMemoryFootprint(const PID &node_pid) const {
  ...
  if (node->IfInnerNode()) {
    ...
    CreateInnerNodeView(node, &keys_view, &children_view, &left_view,
                        &right_view);
    for (PID child : children_view) size += GetMemoryFootprint(child);
  }
...
bool BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
            ValueComparator, ValueEqualityChecker,
            Duplicate>::CompactNode(const PID &node_pid) {
  const BWNode<KeyType, KeyComparator> *node = pid_table_.get(node_pid);
  ...
  if (node->IfInnerNode()) {
    ...
    CreateInnerNodeView(node, &keys_view, &children_view, &left_view,
                        &right_view);
    for (PID child : children_view) CompactNode(child);
  }
  return true;
}
...
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
            ValueComparator, ValueEqualityChecker,
            Duplicate>::PrintSelf(__attribute__((unused)) PID pid,
...
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker, class ValueComparator,
          class ValueEqualityChecker, bool Duplicate>
const BWNode<KeyType, KeyComparator> *BWTree<
    KeyType, ValueType, KeyComparator, KeyEqualityChecker, ValueComparator,
    ValueEqualityChecker,
    Duplicate>::SplitInnerNode(const BWNode<KeyType, KeyComparator> *inner_node,
                               const PID &node_pid) {
  myassert(inner_node->IfInnerNode());
  std::vector<KeyType> keys_view;
  std::vector<PID> children_view;
  PID left_view, right_view;
  CreateInnerNodeView(inner_node, &keys_view, &children_view, &left_view,
                      &right_view);
  ...
  const BWNode<KeyType, KeyComparator> *new_node =
      new BWInnerNode<KeyType, KeyComparator>(
          ...
          std::vector<PID>(children_view.cbegin() + index,
                           children_view.cend()),
          node_pid, right_view, split_key, inner_node->GetHighKey(), true,
          inner_node->HasHighKey());
  PID new_pid = pid_table_.allocate_PID(new_node);

  const BWNode<KeyType, KeyComparator> *split_node =
      new BWSplitNode<KeyType, KeyComparator>(inner_node, index - 1, left_view,
                                              new_pid, split_key, new_pid);
...
const BWNode<KeyType, KeyComparator> *BWTree<
    KeyType, ValueType, KeyComparator, KeyEqualityChecker, ValueComparator,
    ValueEqualityChecker,
    Duplicate>::SplitLeafNode(const BWNode<KeyType, KeyComparator> *leaf_node,
                              const PID &node_pid) {
  ...
  if (!Duplicate) {
    ...
    const BWNode<KeyType, KeyComparator> *new_node =
        new BWLeafNode<KeyType, KeyComparator, ValueType>(
            std::vector<KeyType>(keys_view.cbegin() + index, keys_view.cend()),
            std::vector<ValueType>(values_view.cbegin() + index,
                                   values_view.cend()),
            node_pid, right_view, split_key, leaf_node->GetHighKey(), true,
            leaf_node->HasHighKey());
    PID new_pid = pid_table_.allocate_PID(new_node);
    const BWNode<KeyType, KeyComparator> *split_node =
        new BWSplitNode<KeyType, KeyComparator>(leaf_node, index, left_view,
                                                new_pid, split_key, new_pid);
    ...
    const BWNode<KeyType, KeyComparator> *new_node =
        new BWLeafNode<KeyType, KeyComparator, std::vector<ValueType>>(
            std::vector<KeyType>(keys_view.cbegin() + index, keys_view.cend()),
            std::vector<std::vector<ValueType>>(values_view.cbegin() + index,
                                                values_view.cend()),
            node_pid, right_view, split_key, leaf_node->GetHighKey(), true,
            leaf_node->HasHighKey());
    PID new_pid = pid_table_.allocate_PID(new_node);
...
BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker, ValueComparator,
       ValueEqualityChecker,
       Duplicate>::Split(const BWNode<KeyType, KeyComparator> *node_ptr,
                         const PID &node_pid) {
  ...
  while (true) {
    ...
    if (node_ptr->IfInnerNode()) {
      ...
      if (ExistKeyValue(node_ptr, key, value, value_vector_size)) {
        ...
        if (!DeltaInsert(current, node_ptr, key, value,
                         value_vector_size == 0)) {
          continue;
        }
        result = true;
...
bool BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
            ValueComparator, ValueEqualityChecker,
            Duplicate>::DeleteEntry(const KeyType &key,
                                    const ValueType &value) {
  EpochTime time = GarbageCollector::global_gc_.Register();
  std::vector<PID> path = {root_};
  VersionNumber root_version_number = root_version_number_;
...
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
            ValueComparator, ValueEqualityChecker,
            Duplicate>::ScanKey(const KeyType &key,
  ...
  while (true) {
    ...
    if (node_ptr->IfInnerNode()) {
      ...
      if (!Duplicate) {
        ...
        std::vector<ValueType> values_view;
        PID left_view, right_view;
        CreateLeafNodeView(node_ptr, &keys_view, &values_view, &left_view,
                           &right_view);
        auto position = std::lower_bound(keys_view.begin(), keys_view.end(),
                                         key, key_comparator_);
        if (position == keys_view.end() ||
            !key_equality_checker_(key, *position))
          break;
        myassert((position == keys_view.begin() ||
...
bool BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
            ValueComparator, ValueEqualityChecker,
            Duplicate>::CheckStatus(const BWNode<KeyType, KeyComparator> *
                                        node_ptr,
...
bool BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
            ValueComparator, ValueEqualityChecker,
            Duplicate>::ExistKeyValue(const BWNode<KeyType, KeyComparator> *
                                          node_ptr,
                                      const KeyType &key,
                                      const ValueType &value,
  ...
  while (true) {
    ...
    switch (node_ptr->GetType()) {
      case NLeaf: {
        if (Duplicate) {
          const BWLeafNode<KeyType, KeyComparator,
                           std::vector<ValueType>> *leaf_node =
              static_cast<const BWLeafNode<KeyType, KeyComparator,
                                           std::vector<ValueType>> *>(node_ptr);
          ...
          // find the key
          auto position = std::lower_bound(keys.cbegin(), keys.cend(), key,
                                           key_comparator_);
          if (position != keys.cend() &&
              key_equality_checker_(key, *position)) {
            // if the key exists
            auto dist = std::distance(keys.cbegin(), position);
            myassert(values[dist].size() != 0);
            value_vector_size += values[dist].size();
            if (count == 0) {
              // if we haven't found the corresponding value with this key
              const std::vector<ValueType> &value_vector = values[dist];
              auto value_position =
                  std::lower_bound(value_vector.cbegin(), value_vector.cend(),
                                   value, value_comparator_);
              return (value_position != value_vector.cend()) &&
                     value_equality_checker_(*value_position, value);
            }
          }
        } else {
          const BWLeafNode<KeyType, KeyComparator, ValueType> *leaf_node =
              static_cast<
                  const BWLeafNode<KeyType, KeyComparator, ValueType> *>(
                  node_ptr);
          const std::vector<KeyType> &keys = leaf_node->GetKeys();
          ...
          if (position != keys.cend() &&
            ...
            ++value_vector_size;
            if (count == 0)
              return value_equality_checker_(
                  value, values[std::distance(keys.cbegin(), position)]);
          }
        }
        return count > 0;
      }
      case NInsert: {
        ...
        if (key_equality_checker_(key, insert_node->GetKey())) {
          ++value_vector_size;
          if (value_equality_checker_(value, insert_node->GetValue())) ++count;
        }
      ...
      case NDelete: {
        ...
        if (key_equality_checker_(key, delete_node->GetKey())) {
          --value_vector_size;
          if (value_equality_checker_(value, delete_node->GetValue())) --count;
        }
        node_ptr = node_ptr->GetNext();
        break;
      }
  ...
  while (true) {
    switch (node_ptr->GetType()) {
      case NInner: {
        ...
        auto position =
            std::upper_bound(keys.cbegin(), keys.cend(), key, key_comparator_);
        auto dist = std::distance(keys.cbegin(), position);
        return children[dist];
      }
      case NSplitEntry: {
        const BWSplitEntryNode<KeyType, KeyComparator> *split_entry_node =
...
bool BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
  ...
  while (true) {
    switch (node->GetType()) {
      case NInner: {
        const BWInnerNode<KeyType, KeyComparator> *inner_node =
            static_cast<const BWInnerNode<KeyType, KeyComparator> *>(node);
        std::vector<KeyType> keys = inner_node->GetKeys();
        auto position = std::upper_bound(keys.cbegin(), keys.cend(), split_key,
                                         key_comparator_);
        if (position != keys.cbegin() &&
            key_equality_checker_(split_key, *(position - 1)))
          // some one else has inserted the split entry node
          return true;
        if (position != keys.cend()) {  // find a high key
          if (!has_high_key) {
            has_high_key = true;
            high_key = *position;
          } else if (key_comparator_(*position, high_key))
            high_key = *position;
        }
        ...
        if (has_high_key)
          split_entry = new BWSplitEntryNode<KeyType, KeyComparator>(
              parent_node, split_key, high_key, split_to);
        else
          split_entry = new BWSplitEntryNode<KeyType, KeyComparator>(
              parent_node, split_key, split_to);
      ...
      case NSplitEntry: {
        const BWSplitEntryNode<KeyType, KeyComparator> *split_entry_node =
            static_cast<const BWSplitEntryNode<KeyType, KeyComparator> *>(node);
        if (key_equality_checker_(split_key, split_entry_node->GetToLowKey()))
          // some one else has inserted the split entry node
          return true;
        if (key_comparator_(split_key, split_entry_node->GetToLowKey())) {
          // find a high key
          if (!has_high_key) {
            has_high_key = true;
            high_key = split_entry_node->GetToLowKey();
          } else if (key_comparator_(split_entry_node->GetToLowKey(), high_key))
            high_key = split_entry_node->GetToLowKey();
        }
        node = node->GetNext();
        break;
...
/*
 * insert a split entry node at the parent of 'top' to finish the second step of
 * split
 * path contains the pids all the way from the root down to 'top'
 * if the second step is finished (either by us or by others), this function
 * returns true
 */
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker, class ValueComparator,
...
bool BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
            ValueComparator, ValueEqualityChecker, Duplicate>::
    InsertSplitEntry(const BWNode<KeyType, KeyComparator> *top,
                     const KeyType &key, std::vector<PID> &path,
                     const VersionNumber &root_version_number) {
  ...
  if (path.size() > 1) {  // In this case, this is a normal second step split
    ...
    while (true) {
      ...
      if ((path.size() == 2 && root_version_number != root_version_number_) ||
        ...
        return false;
      if (FindOrInsertSplitEntry(split_key, to, parent_node, pid)) return true;
    }
  } else {
...
bool BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
            ValueComparator, ValueEqualityChecker,
            Duplicate>::Consolidate(const PID &cur,
                                    const BWNode<KeyType, KeyComparator> *
                                        node_ptr) {
...
/*
 * Create a consolidated inner node logically equivalent to the argument
 * node_chain.
 * Return the new node
 */
template <typename KeyType, typename ValueType, class KeyComparator,
...
const BWNode<KeyType, KeyComparator> *
BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker, ValueComparator,
       ValueEqualityChecker, Duplicate>::
...
/*
 * Create a consolidated leaf node logically equivalent to the argument
 * node_chain.
 * Return the new node
 */
template <typename KeyType, typename ValueType, class KeyComparator,
...
/*
 * Construct vector keys and values from a leaf node linked list, along with its
 * left and right siblings.
 * Collect everthing from node_chain and put it to (or remove it from) keys and
 * values.
 */
template <typename KeyType, typename ValueType, class KeyComparator,
          class KeyEqualityChecker, class ValueComparator,
  ...
  if (node_chain->GetType() == NLeaf) {
    ...
    const BWLeafNode<KeyType, KeyComparator, ValueType> *node =
        static_cast<const BWLeafNode<KeyType, KeyComparator, ValueType> *>(
            node_chain);
    if (keys != nullptr) *keys = node->GetKeys();
    if (values != nullptr) *values = node->GetValues();
    if (left != nullptr) *left = node->GetLeft();
    if (right != nullptr) *right = node->GetRight();
    return;
  }

  // still at delta chain
  myassert(node_chain->GetNext() != NULL);
  ConstructConsolidatedLeafNodeInternal(node_chain->GetNext(), keys, values,
                                        left, right);
  switch (node_chain->GetType()) {
    case NInsert:
      ConsolidateInsertNode(
          static_cast<const BWInsertNode<KeyType, KeyComparator, ValueType> *>(
              node_chain),
          keys, values);
      break;
    case NDelete:
      ConsolidateDeleteNode(
          static_cast<const BWDeleteNode<KeyType, KeyComparator, ValueType> *>(
              node_chain),
          keys, values);
      break;
    case NSplit:
      ConsolidateSplitNode(
          static_cast<const BWSplitNode<KeyType, KeyComparator> *>(node_chain),
          keys, values, right);
      break;
    case NMerge:
      myassert(0);
...
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
  ...
  if (node_chain->GetType() == NInner) {
    ...
    if (keys != nullptr) *keys = node->GetKeys();
    if (children != nullptr) *children = node->GetChildren();
    if (left != nullptr) *left = node->GetLeft();
    if (right != nullptr) *right = node->GetRight();
    return;
  }
  ...
  // still at delta chain
  myassert(node_chain->GetNext() != NULL);
  ConstructConsolidatedInnerNodeInternal(node_chain->GetNext(), keys, children,
                                         left, right);
  switch (node_chain->GetType()) {
    case NSplit:
      ConsolidateSplitNode(
...
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
  ...
  if (position == keys->begin() ||
    ...
    auto value_position = std::upper_bound(
...
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
  ...
  myassert(position != keys->end() && key_equality_checker_(key, *position) &&
           (position + 1 == keys->end() ||
            !key_equality_checker_(key, *(position + 1))) &&
           (position == keys->begin() ||
            !key_equality_checker_(key, *(position - 1))));
...
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
  ...
  auto begin = std::lower_bound(value_vector.begin(), value_vector.end(), value,
                                value_comparator_);
  auto end = std::upper_bound(value_vector.begin(), value_vector.end(), value,
                              value_comparator_);
...
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
            ValueComparator, ValueEqualityChecker, Duplicate>::
    ConsolidateSplitNode(const BWSplitNode<KeyType, KeyComparator> *node,
                         std::vector<KeyType> *keys,
  ...
  if (right != nullptr) *right = node->GetSplitTo();
  myassert(!Duplicate);
}
...
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
  ...
  auto position =
      std::lower_bound(keys->begin(), keys->end(), key, key_comparator_);
  // check for both existence and uniqueness
  myassert(position != keys->begin() && position != keys->end() &&
           key_equality_checker_(key, *position) &&
           (position + 1 == keys->end() ||
            !key_equality_checker_(key, *(position + 1))) &&
           (position == keys->begin() ||
...
void BWTree<KeyType, ValueType, KeyComparator, KeyEqualityChecker,
  ...
  if (node->HasToHighKey()) {
    ...
    auto position = std::lower_bound(keys->begin(), keys->end(), to_high_key,
                                     key_comparator_);
    // check for both existence and uniqueness
    myassert(position != keys->end() &&
             key_equality_checker_(to_high_key, *position) &&
             (position + 1 == keys->end() ||
              !key_equality_checker_(to_high_key, *(position + 1))) &&
             (position == keys->begin() ||
              !key_equality_checker_(to_high_key, *(position - 1))));
    auto dist = std::distance(keys->begin(), position);
...
template <typename KeyType, typename KeyComparator>
void BWInnerNode<KeyType, KeyComparator>::Print(
    const PIDTable<KeyType, KeyComparator> &, int indent) const {
  std::string s;
  for (size_t i = 0; i < indent; i++) {
    s += "\t";
  }
...
template class BWTree<IntsKey<1>, ItemPointer, IntsComparator<1>,
                      IntsEqualityChecker<1>, ItemPointerComparator,
                      ItemPointerEqualityChecker, true>;

template class BWTree<IntsKey<2>, ItemPointer, IntsComparator<2>,
                      IntsEqualityChecker<2>, ItemPointerComparator,
                      ItemPointerEqualityChecker, true>;

template class BWTree<IntsKey<3>, ItemPointer, IntsComparator<3>,
                      IntsEqualityChecker<3>, ItemPointerComparator,
                      ItemPointerEqualityChecker, true>;

template class BWTree<IntsKey<4>, ItemPointer, IntsComparator<4>,
                      IntsEqualityChecker<4>, ItemPointerComparator,
                      ItemPointerEqualityChecker, true>;

template class BWTree<IntsKey<1>, ItemPointer, IntsComparator<1>,
                      IntsEqualityChecker<1>, ItemPointerComparator,
                      ItemPointerEqualityChecker, false>;
...
template class BWTree<GenericKey<12>, ItemPointer, GenericComparator<12>,
                      GenericEqualityChecker<12>, ItemPointerComparator,
                      ItemPointerEqualityChecker, true>;
...
template class BWTree<GenericKey<24>, ItemPointer, GenericComparator<24>,
                      GenericEqualityChecker<24>, ItemPointerComparator,
                      ItemPointerEqualityChecker, true>;

template class BWTree<GenericKey<32>, ItemPointer, GenericComparator<32>,
                      GenericEqualityChecker<32>, ItemPointerComparator,
                      ItemPointerEqualityChecker, true>;

template class BWTree<GenericKey<48>, ItemPointer, GenericComparator<48>,
                      GenericEqualityChecker<48>, ItemPointerComparator,
                      ItemPointerEqualityChecker, true>;
...
template class BWTree<GenericKey<96>, ItemPointer, GenericComparator<96>,
                      GenericEqualityChecker<96>, ItemPointerComparator,
                      ItemPointerEqualityChecker, true>;

template class BWTree<GenericKey<128>, ItemPointer, GenericComparator<128>,
                      GenericEqualityChecker<128>, ItemPointerComparator,
                      ItemPointerEqualityChecker, true>;
...
template class BWTree<GenericKey<512>, ItemPointer, GenericComparator<512>,
                      GenericEqualityChecker<512>, ItemPointerComparator,
                      ItemPointerEqualityChecker, true>;
